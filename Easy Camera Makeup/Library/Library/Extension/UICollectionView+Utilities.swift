...
//  ChoTot
//
//  Created by Pham Quang Vinh on 11/12/20.
//  Copyright © 2020 Chotot. All rights reserved.
//
...
extension UICollectionView {
    ...
    public func reloadDataSavingSelections() {
        let selectedRows = indexPathsForSelectedItems

        reloadData()

        if let selectedRow = selectedRows {
            for indexPath in selectedRow {
                selectItem(at: indexPath, animated: false, scrollPosition: .bottom)
            }
        }
    }
}
