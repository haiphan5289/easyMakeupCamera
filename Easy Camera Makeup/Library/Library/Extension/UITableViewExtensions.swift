...
//        let emptyView: EmptyView = .loadXib()
//        emptyView.frame = f
...
extension UITableView {
    ...
    func selectAllRows(section: Int) {
        let totalRows = self.numberOfRows(inSection: section)
        for row in 0..<totalRows {
            self.selectRow(at: IndexPath(row: row, section: section), animated: true, scrollPosition: .none)
        }
    }
    
    func deSelectAllRows(section: Int) {
        let totalRows = self.numberOfRows(inSection: section)
        for row in 0..<totalRows {
            self.deselectRow(at: IndexPath(row: row, section: section), animated: true)
        }
    }
